...
ObjLoader::ObjLoader(const char* objName){
	buildMesh(objName);
}
...
void ObjLoader::buildMesh (const char * objName){
...
bool ObjLoader::loadOBJ(const char * path,	stdVec3 & out_vertices, stdVec2 & out_uvs, stdVec3 & out_normals){
	std::vector<unsigned int> vertexIndices, uvIndices, normalIndices;
	stdVec3 temp_vertices;
	stdVec2 temp_uvs;
	stdVec3 temp_normals;
		...
		if (strcmp(lineHeader, "v") == 0){
			...
			if (matches != 9){
				...
				if(matches != 6){
					...
					if(matches != 3){
						("File Can't be read by our simple toy parsing system. Please upgrade it.(Try exporting with other options)\n");
						return false;
					}
				}
			}
